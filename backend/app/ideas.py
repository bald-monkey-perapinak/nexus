...
# Limit concurrent discriminator calls to avoid quota exhaustion
DISC_SEMAPHORE = asyncio.Semaphore(3)
...
GENERATOR_PROMPT = """Ты — эксперт по бизнес-идеям с 20-летним опытом. Генерируй только ЗАКОННЫЕ бизнес-идеи.
...
6. СТРОГО: только легальный бизнес, соответствующий законодательству РФ
...
OPS_DISC_PROMPT = """Операционный директор и юридический консультант. Оцени реалистичность запуска и легальность.
Ответь ТОЛЬКО JSON без markdown.
...
ВАЖНО: если идея связана с незаконной деятельностью, мошенничеством, обходом законов,
финансовыми пирамидами, продажей запрещённых веществ или любой другой нелегальной деятельностью —
обязательно верни verdict="fail" и флаг "illegal_activity".

{{"verdict":"pass|warn|fail","score":75,"reason":"одно предложение","flags":[]}}

Доступные флаги: license_required, license_medical, license_education, license_alcohol,
team_insufficient, experience_gap, high_ops_complexity, illegal_activity"""
...
async def generate_candidates(state: IdeaGenerationState) -> IdeaGenerationState:
    ...
    try:
        ...
        content = await ainvoke_with_fallback(prompt, tier="heavy", temperature=0.7, max_tokens=4096)
        ...
        return {**state, "errors": [sanitize_exception(e, "generation_failed")], "status": "error"}
...
async def _discriminate(idea: dict, profile: dict, disc_type: str) -> dict:
    ...
    try:
        async with DISC_SEMAPHORE:
            if disc_type == "financial":
                prompt = FINANCIAL_DISC_PROMPT.format(
                    title=idea["title"], description=idea.get(
                        "description", ""),
            ...
            content = await ainvoke_with_fallback(prompt, tier="fast", temperature=0.1, max_tokens=400)
        ...
        log.error("Discriminator failed", disc_type=disc_type,
                  idea=idea.get("id"), error=str(e)[:200])
        return {"verdict": "warn", "score": 50, "flags": ["llm_error"]}
...
def aggregate_scores(state: IdeaGenerationState) -> IdeaGenerationState:
    ...
    for idea in state["idea_candidates"]:
        ...
        # Жёсткий фильтр: незаконная деятельность
        if "illegal_activity" in all_flags:
            continue
...
async def enrich_cards(state: IdeaGenerationState) -> IdeaGenerationState:
    ...
    async def enrich_one(idea: dict) -> dict:
        try:
            ...
            log.error("enrich_card failed", idea=idea.get(
                "id"), error=str(e)[:200])
            return idea

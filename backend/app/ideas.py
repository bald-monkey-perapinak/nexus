...
GENERATOR_PROMPT = """Ты — старший бизнес-аналитик с опытом 20 лет.
...
- Опыт: {experience}
- ИСКЛЮЧЕНИЯ (категорически не хочет): {exclusions}
- Уровень технологичности: {tech_level}
- Риск-профиль: {risk_profile}
- Основной доход: {is_main_income}
- Уже есть: клиенты={has_clients}, помещение={has_premises}, партнёры={has_partners}

ПРОТИВОРЕЧИЯ В ПРОФИЛЕ (учти при генерации):
{contradictions_summary}

ПРАВИЛА ГЕНЕРАЦИИ:
1. Ровно 10 идей — разных ниш, без повторов
2. НИ ОДНА идея не должна касаться исключений (даже косвенно)
3. Если опыт пользователя совпадает с исключением — используй навыки, но не сферу
   Пример: опыт в общепите + исключение "общепит" → идея "автоматизация ресторанов" (B2B, не общепит)
4. Минимум 2 идеи — прямое применение опыта (если нет конфликта с исключениями)
5. Минимум 1 идея — нестандартная, неочевидная
6. Стартовые затраты ≤ 80% от капитала пользователя
7. Команда на старте ≤ указанной пользователем
8. Для offline/hybrid — учти реальную аренду в городе тира {city_tier}
9. Для города тира 1 (Москва/СПб): чек выше на 40-50%, аренда дороже в 2-3 раза
10. Для города тира 3 (малые города): рынок меньше, чек ниже на 20-30%
...
[
  {{
    ...
    "description": "Суть бизнеса (2-3 предложения, конкретно)",
    "relevance_explanation": "Почему подходит этому человеку (1 предложение)",
    "main_risk": "Главный конкретный риск с цифрами",
    "success_factor": "Ключевой фактор успеха (конкретно)",
    "market_analogues": ["Реальный пример 1", "Реальный пример 2"],
...
def _check_stop_factors_deterministic(idea: dict, profile: dict) -> list[str]:
    ...
    title_desc = (idea.get("title", "") + " " + idea.get("description", "")).lower()
    exclusion_keywords = [w.strip() for w in exclusions.split(",") if len(w.strip()) > 2]
...
async def run_discriminators(state: IdeaGenerationState) -> IdeaGenerationState:
    ...
    for idea, result in zip(candidates, results):
        if isinstance(result, Exception):
            result = {"verdict": "warn", "total_score": 40, "flags": [], "realism_warnings": []}
        scored.append({
            **idea,
            "checklist": result,

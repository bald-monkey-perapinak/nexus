...
llm = ChatGroq(model=settings.GROQ_MODEL, api_key=settings.GROQ_API_KEY, temperature=0.4, max_tokens=4096)
...
class RoadmapState(TypedDict):
    ...
    idea: dict
    profile: dict
    financial: dict          # assumptions from financial model
    all_flags: List[str]
...
def load_flag_tasks(state: RoadmapState) -> RoadmapState:
    ...
    for flag in state.get("all_flags", []):
        for t in FLAG_TASKS.get(flag, []):
            tasks.append({**t, "id": f"flag_{flag}", "source_flag": flag, "status": "todo"})
...
async def generate_base_roadmap(state: RoadmapState) -> RoadmapState:
    ...
    try:
        prompt = ROADMAP_PROMPT.format(
            ...
            startup_costs=assumptions.get("startup_costs", {}).get("total", "не указаны"),
            fixed_costs=assumptions.get("fixed_costs_monthly", {}).get("total", "не указаны"),
            avg_check=assumptions.get("avg_check_rub", "не указан"),
            existing_tasks=", ".join(existing) if existing else "нет",
        )
        resp = await llm.ainvoke(prompt)
        tasks = _parse(resp.content)
        if not isinstance(tasks, list):
            raise ValueError("Expected array")
        # Add status and ensure id
        tasks = [{**t, "status": "todo", "id": t.get("id", f"base_{i}")} for i, t in enumerate(tasks)]
        return {**state, "base_tasks": tasks}
    except Exception as e:
        return {**state, "base_tasks": [], "errors": [f"roadmap_gen_failed:{str(e)[:200]}"]}
...
def merge_and_finalize(state: RoadmapState) -> RoadmapState:
    ...
    for t in all_tasks:
        ...
        if key not in seen:
            ...
            deadline = (start_date + timedelta(weeks=week)).strftime("%Y-%m-%d")
            unique.append({**t, "week": week, "deadline": deadline})

    # Sort: week asc, critical first
    priority_order = {"critical": 0, "important": 1, "optional": 2}
    unique.sort(key=lambda x: (x["week"], priority_order.get(x.get("priority","optional"), 2)))
